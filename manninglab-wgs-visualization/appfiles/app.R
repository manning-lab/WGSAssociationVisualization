...
makePlot <- function(temp, input, output)
{
  ...
  ldpath <- ifelse(input$ldpath=="", "1kg-t2d.chr20_60.9M-61.1M.ld.csv", paste0(input$bucket, "/", isolate(input$ldpath)))
  ...
  {
    write(system(paste("/usr/local/gcloud/google-cloud-sdk/bin/gsutil cat", ldpath), intern=T), "ldFile.txt")
  ...
  tabPanel("tab",
           div( id ="Sidebar",
                sidebarPanel(
                  ...
                  fluidRow(
                    textInput("bucket", h4("Google bucket link"), value="", placeholder = "gs://fc-91605a4c-df34-4248-b17v-ca123456e59"),
                  ...
                  fluidRow(
                    ...
                    column(5, textInput("chr","Chromosome", value="", placeholder="2"))
                  ),
                  fluidRow(
                    column(5, textInput("pos","Position", value="", placeholder="3")),
                    column(5, textInput("pval","P-value", value="", placeholder="9"))
                  ),
                  fluidRow(
                    ...
                    #      textInput("ldpath", h4("LD file path(optional)"), value="", placeholder = "gs://fc-91605a4c-df34-4248-b17v-ca123456e59/ld-data-file.ld.csv"),
           ...
           mainPanel(
             ...
             bsAlert("alert"),
             plotOutput("plot", width="120%", height="500"),
             tableOutput("topvars")
           )
  )
)
...
server <- function(input, output, session) 
{
  ...
  output$plot <- renderPlot(make_regional_plot(chr=20, start=60900000, end=61100000, variant_data=temp, 
                                               variant_chr_column = "V2", variant_pos_column = "V3", variant_y_column = "V9", 
                                               variant_marker_column = "V1", genome_build = "hg19",
  ...
  observeEvent(input$bucketsubmit, {
    ...
    js <- fromJSON(file = "/tmp/application_default_credentials.json")
    boto_data <- paste0("[OAuth2]\nclient_id = ", js$client_id, "\nclient_secret = ", js$client_secret, "\n\n[Credentials]\ngs_oauth2_refresh_token = ", js$refresh_token)
    write(boto_data, file="/tmp/.boto")
    write(system("gcloud auth application-default print-access-token", intern=T), "access_token.txt")

    bucket_items <- data.frame(name = as.character(read.table(pipe(paste("/usr/local/gcloud/google-cloud-sdk/bin/gsutil ls", isolate(input$bucket))))$V1),
    ...
    for(i in bucket_items$name)
    {
      if(grepl(".gz$", i))
      {
        ...
        {
          opt_list <- append(opt_list, substring(i, sapply(gregexpr("\\/", i), tail, 1)+1))
        }
      }

    }
    updateSelectInput(session, "gspath", choices = opt_list, selected = "")
    ...
    updateSelectInput(session, "ldpath", choices = opt_list, selected = "")
  })
  ...
  output$plot <- renderPlot({
    ...
    else
    {
      ...
      command <- isolate(paste0("export GCS_OAUTH_TOKEN=", accesstoken," ; /usr/local/htslib-1.9/bin/tabix ", gspath," ", isolate(input$searchrange)))
